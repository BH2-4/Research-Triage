...
当前版本没有真正的“暂停生成”或“停止生成”功能。用户发起一轮对话后，前端会等待 `POST /api/chat` 返回完整响应；在等待期间，输入框、选项按钮、撤销和新对话按钮会被禁用，但不会取消已经发出的请求。

这意味着：

- 点击“处理摘要”只会展开或折叠处理信息，不会暂停 AI 调用。
- 页面没有取消按钮，也没有 `AbortController` 或服务端请求取消机制。
- MiniMax 等外部模型调用在服务端继续执行，直到返回、报错或网络请求结束；前端无法提前停止等待。
- 如果模型响应较慢，用户只能等待当前请求完成。当前实测部分真实模型请求耗时约 6-95 秒。
- 在请求完成前，撤销操作不可用；请求完成后，“撤销”只能回退前端当前会话快照，不能取消已经发生的模型调用，也不保证撤回已经写入 `userspace/` 的服务端产物。
...
1. “处理摘要”：查看或隐藏单轮处理摘要。
2. “撤销”：请求完成后，将前端消息、画像和 Plan 恢复到上一轮快照。
3. “新对话”：清理当前浏览器会话并生成新的会话 ID。

这些操作都不是请求级别的暂停或取消。刷新页面可以恢复 `sessionStorage` 中保存的消息、画像和 Plan，但撤销历史保存在 React 内存中，刷新后不会恢复。

### 后续实现建议

若要支持真正的暂停/停止生成，需要同时改造前端、服务端和模型调用层：

1. 前端使用 `AbortController` 保存当前请求，在加载状态下提供“停止生成”按钮。
2. `/api/chat` 接收并识别取消信号，避免在请求已终止后继续更新会话状态。
3. AI Provider 支持流式响应和请求取消，并在 MiniMax 侧确认取消语义与计费规则。
4. 约定中断时的持久化策略，禁止把半截回复或不完整 Plan 当作正式产物写入 `userspace/`。
5. 增加“发送中、已停止、已完成、失败、可重试”等状态，以及对应的 API、浏览器和持久化测试。
