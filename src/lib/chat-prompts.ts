...
【questions 规则】
- 每个选项是一句完整的、确定的话，用户点击即选中；选项数量按真实需要输出，不要求固定 4 个
- 禁止占位符文本（如"选项A""其他""请选择"）
- 必须且只能包含一个"我不太理解这些，帮我找方向"作为最后一项（用户不知道怎么选时的逃生通道）
...
【questions 规则】
- 输出完整、确定的选项，每个选项是一句用户点击即选中的完整句子；选项数量按真实需要输出，不要求固定 4 个
- 选项必须互斥、覆盖用户可能的选择范围
- 如果同一个问题允许用户同时选择多个兴趣、工具、困难或方向，在 choiceGroups 中输出 mode="multiple"
- 如果同一轮需要确认两组及以上信息，必须使用 choiceGroups，每组一个 prompt；不要把多个 prompt 和选项混在同一个 questions 数组里
- 如果只有一组单选，choiceGroups 可为空数组，系统会使用 questions 渲染单选按钮
- 禁止占位符文本（如"选项C""其他""请选择"）
- 每组必须且只能包含一个"我不太理解这些，帮我找方向"作为最后一个逃逸选项
...
- 列出假设时，每个假设渲染为一个可确认项
- 继续追问时，选项具体到缺失的信息
- 选项数量按真实需要输出，不要求固定 4 个
- 必须且只能包含一个"我不太理解这些，帮我找方向"作为最后一个 question
- 如果同一轮需要确认两组及以上信息，必须使用 choiceGroups，每组一个 prompt；不要把多个 prompt 和选项混在同一个 questions 数组里
